...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (action === 'edit') {
            if (modalTitle) modalTitle.innerHTML = `<i class="ti ti-key me-1"></i> Atur Hak Akses Role: ${role.name}`;
            if (btnSubmitForm) btnSubmitForm.innerHTML = '<i class="ti ti-device-floppy me-1"></i> Simpan Hak Akses';

        } else if (action === 'view') {
            if (modalTitle) modalTitle.innerHTML = `<i class="ti ti-eye me-1"></i> Detail Hak Akses Role: ${role.name}`;
