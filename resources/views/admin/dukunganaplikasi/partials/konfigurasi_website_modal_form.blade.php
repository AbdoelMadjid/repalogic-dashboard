...
@php
    $isMulti = $activeTheme ? $activeTheme->isMultipage() : false;
@endphp
        ...
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-primary text-white py-3">
                <h5 class="modal-title text-white fw-bold" id="modalTambahSeksiLabel">
                    <i class="ti ti-plus me-1"></i> {{ $isMulti ? 'Tambah Halaman Baru' : 'Tambah Seksi Halaman Baru' }}
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.dukunganaplikasi.konfigurasi-website.store-section') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="add_section_name" class="form-label fw-semibold text-dark">{{ $isMulti ? 'Nama Halaman / Modul' : 'Nama Seksi Halaman' }} <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="add_section_name" name="section_name" placeholder="{{ $isMulti ? 'Contoh: Program Akademik' : 'Contoh: Banner Promo Spesial' }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control font-monospace" id="add_section_file" name="section_file" placeholder="{{ $isMulti ? 'page-programs-1.blade.php' : 'section-promo.blade.php' }}" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="add_nav_title" name="nav_title" placeholder="{{ $isMulti ? 'Contoh: Programs' : 'Contoh: Promo' }}">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="mb-3">
                                <label for="add_target_id" class="form-label fw-semibold text-dark">{{ $isMulti ? 'Route Slug / URL' : 'Anchor Target ID (#)' }}</label>
                                <input type="text" class="form-control font-monospace" id="add_target_id" name="target_id" placeholder="{{ $isMulti ? 'programs' : 'promo' }}">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="mb-3">
                    ...
                    @if ($isMulti)
                        <input type="hidden" name="bg_type" value="default">
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                <div class="alert alert-secondary py-2 px-3 mb-0 fs-12 text-muted d-flex align-items-center gap-2">
                                    <i class="ti ti-info-circle fs-16 text-primary"></i>
                                    <span>Pada tema <strong>Multi-Page Portal</strong>, styling visual &amp; layout halaman diatur langsung secara mandiri di dalam berkas Blade dan CSS tema.</span>
                                </div>
                            </div>
                        </div>
                    @else
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex align-items-center gap-4 mt-1">
                                    <div class="form-check form-switch">
                                        ...
                                        <label class="form-check-input-label fw-semibold text-dark ms-1" for="add_is_active">{{ $isMulti ? 'Publikasikan Halaman' : 'Aktifkan Seksi' }}</label>
                                    </div>
                                    <div class="form-check form-switch">
                ...
                <div class="modal-footer bg-light py-3">
                    <button type="button" class="btn btn-secondary px-3" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn btn-primary px-4 fw-semibold">
                        <i class="ti ti-plus me-1"></i> {{ $isMulti ? 'Simpan Halaman' : 'Simpan Seksi' }}
                    </button>
                </div>
            </form>
        </div>
...
<div class="modal fade" id="modal-edit-seksi" tabindex="-1" aria-labelledby="modalEditSeksiLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-primary text-white py-3">
                <h5 class="modal-title text-white fw-bold" id="modalEditSeksiLabel">
                    <i class="ti ti-edit me-1"></i> {{ $isMulti ? 'Edit Halaman Website' : 'Edit Seksi Halaman' }}
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="" method="POST" id="form-edit-seksi" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="edit_section_name" class="form-label fw-semibold text-dark">{{ $isMulti ? 'Nama Halaman / Modul' : 'Nama Seksi' }} <span class="text-danger">*</span></label>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label for="edit_target_id" class="form-label fw-semibold text-dark">{{ $isMulti ? 'Route Slug / URL' : 'Anchor Target ID (#id)' }}</label>
                                <input type="text" class="form-control font-monospace" id="edit_target_id" name="target_id">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex align-items-center gap-4 mt-1">
                                    <div class="form-check form-switch">
                                        ...
                                        <label class="form-check-input-label fw-semibold text-dark ms-1" for="edit_is_active">{{ $isMulti ? 'Publikasikan Halaman' : 'Aktifkan Seksi' }}</label>
                                    </div>
                                    <div class="form-check form-switch">
